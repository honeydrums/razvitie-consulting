...
commentTextarea.oninput = function() { // функция, следящая за вводом текста в textarea
    textareaQuantity.innerHTML = commentTextarea.value.length; // счётчик количества введённых символов
    if (commentTextarea.value.length > 500) { // условие, если больше 500 символов в поле ввода
        formButton.disabled = true; // отключим кнопку и возможность отправки данных
        formButton.classList.add('too-many-symbols__button');
        commentTextarea.classList.add('too-many-symbols__textarea'); // добавим рамку и красный цвет текста
        ...
        commentTextarea.classList.remove('too-many-symbols__textarea'); // удалим рамку и красный цвет текста
